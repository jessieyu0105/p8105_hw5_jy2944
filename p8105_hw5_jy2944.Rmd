...
The problem focuses on a longitudinal study that included an experimental group and a control group. 

### Import and tidy data

We use `purrr::map` to read the data from each csv and combine them into one dataset. 

After tidying, each row in the resulting dataset represents the information (group assignment, id, observation value) for each subject in each week (8 weeks in total).
...
study_df %>% 
  ...
  ggplot(aes(x = week, y = observation_value , color = group, type = subject_id)) + 
  geom_point() +
  geom_line() +
  labs(
